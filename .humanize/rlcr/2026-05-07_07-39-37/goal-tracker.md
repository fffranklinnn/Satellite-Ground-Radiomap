...
|------|-----------|--------|-----|-------|-------|
| task5 | AC-3 | completed | coding | claude | `main.py` now resolves policy before layer construction so scene-disabled layers are not instantiated |
| task12 | AC-3 | completed | coding | claude | Multisat strict-mode checks now normalize project-root-relative input paths and accept nested `layers.l1_macro.tle.file` |
...
|-------|-----------------|-------------|-----------------|
| Multisat strict required-data validation ignores nested `layers.l1_macro.tle.file` after path normalization | 1 | AC-3 | Update `check_required_data` to use the same TLE extraction semantics as policy and manifest collection, then add a cwd-changing strict regression for nested `tle.file` |
| `main.py` initializes L2/L3 from raw config before resolving layer policy | 1 | AC-3 | Resolve policy before layer construction and instantiate only policy-enabled layers on the canonical path; add a regression proving scene-disabled layers are not constructed |
...
|----|------|-----------------|----------------|----------|
| AC-3 | task5 | 2 | 2 | `pytest tests/test_runtime_layer_policy.py` passed with a regression asserting policy-disabled layers are not constructed |
| AC-3 | task12 | 2 | 2 | `pytest tests/test_runtime_layer_policy.py` passed with nested `tle.file` strict-path coverage |
