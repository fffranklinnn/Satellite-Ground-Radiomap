...
class TestDeterministicSelection:
    ...
    def test_equal_elevation_tiebreak_lowest_norad(self):
        """When elevations are equal, the real select() sort returns lowest numeric NORAD ID."""
        from unittest.mock import patch

        sel = SatelliteSelector(TLE_FIXTURE, strict=True, min_elevation_deg=-90.0)

        # Stub _collect_candidates to return controlled equal-elevation candidates.
        # This exercises the real select() sort logic, not a hand-written lambda.
        fake_candidates = [
        ...
        with patch.object(sel, '_collect_candidates', return_value=fake_candidates):
            result = sel.select(TS, CENTER)

        assert result["norad_id"] == "11111", (
            f"Expected lowest NORAD 11111 under equal elevation, got {result['norad_id']}"
        )
